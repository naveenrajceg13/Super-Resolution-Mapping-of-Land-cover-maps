function ant = pso2(class_number)
...
%         display(spsam);
        ...
        for k=1:proportion
           xmatrix(sort_position_x(1,k),sort_position_y(1,k))=class_number; 
        end
        for ii=i:i+(mapsize-1)
            for jj=j:j+(mapsize-1)
                if xmatrix(ii,jj)~=class_number
                    xmatrix(ii,jj)=0.0;
                end
            end
        end
...
display(xmatrix);
ant=xmatrix;
str = int2str(class_number);
imwrite(ant,strcat(str,'-ant.jpg'),'jpeg');
